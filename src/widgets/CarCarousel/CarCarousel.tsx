...
const CarCard: React.FC<CarCardProps> = ({ car, isActive }) => {
  ...
  const image =
    car.images && car.images.length > 0 ? car.images[0] : bmwFallback;
...
const CarCarousel: React.FC<CarCarouselProps> = ({ initialFilter = {} }) => {
  ...
  useEffect(() => {
    const hasSeenIndicator =
      localStorage.getItem("hasSeenSwipeIndicator") === "true";
  ...
  const canGoNext =
    activeIndex < filteredCars.length - (isMobile ? 1 : visibleCards);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    ...
    const basePosition = isMobile
      ? -(activeIndex * cardWidth) +
        ((trackRef.current?.clientWidth || 0) - cardWidth) / 2
      : -(activeIndex * cardWidth);
    ...
    if (
      ...
      adjustedDelta =
        delta * (0.5 - 0.2 * (Math.min(Math.abs(delta), 150) / 150));
    }
        ...
        <div className={styles.carouselTrack}>
          ...
          >
            {filteredCars.map((car, index) => (
              <div key={car.id || index} className={styles.carouselItem}>
